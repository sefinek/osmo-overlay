...
`RenderOptions.RangeStartSeconds/RangeEndSeconds` (GUI: In/Out on the preview timeline, `MainWindow.Range.cs`; CLI: `--from/--to`) become a `RenderRange` of whole frames on the combined timeline. `FfmpegPipeline.AddSourceInputs` feeds a range that starts partway in as: the segment it starts in with a plain `-ss` (video + audio), plus - only if it runs on into later segments - a concat input of those (video, joined with the concat filter) and an audio-only concat list (`ConcatListWriter.WriteAudioOnly`, needs `exact_stream_id` and `-itsoffset` = the list's probed start time, see its XML comment for why). Verified on real Osmo files: first frame exact, seam frame-exact, audio 0 ms off. Don't switch this to concat's own `inpoint`/`-ss` for video (stuck frames) or drop the audio list's `-itsoffset` (audio leads by ~0.3-0.8 s). creation_time and the timecode (`SmpteTimecode`, drop-frame aware) move to the range's first frame; the route intro, watermark and widget Appear/Disappear timing count from the range start (`OverlayRenderer.TimelineOffsetSeconds`), data widgets stay on recording time. A partial render doesn't copy the camera's djmd/dbgi tracks (they'd cover the whole recording).
