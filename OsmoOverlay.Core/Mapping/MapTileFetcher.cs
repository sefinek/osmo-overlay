...
/// <summary>
///     Downloads XYZ map tiles and caches them on disk indefinitely (map tiles for a given
///     z/x/y don't change in any way that matters here), so re-rendering the same route - or an
///     overlapping one - doesn't re-fetch tiles that are already on disk. Identifies itself with a
///     real User-Agent and caches aggressively, per OpenStreetMap's tile usage policy
///     (https://operations.osmfoundation.org/policies/tiles/) for the default tile source; a
///     custom MapTileUrlTemplate is the user's own responsibility to use within its provider's terms.
/// </summary>
public static class MapTileFetcher
{
	...
	public static async Task<SKBitmap?> FetchAsync(string urlTemplate, int zoom, int x, int y, CancellationToken ct)
	{
		...
		try
		{
			...
			{
				using SKBitmap? cached = SKBitmap.Decode(cachePath);
